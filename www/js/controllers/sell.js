app.controller('SellCtrl', ['$scope', '$rootScope', '$categories', '$ionicLoading', '$message', function($scope, $rootScope, $categories, $ionicLoading, $message) {
  ...
  $scope.GetSubs = function(){
    return $categories.GetSubs($scope.s.main_category);
  }
  ...
  $scope.OpenFile = function(){
    ionic.trigger('click', { target: document.getElementById('sell_items_select') });
  ...
  $scope.ShowLoading = function() {
    $ionicLoading.show({
      template: '<ion-spinner icon="lines" class="spinner-calm"></ion-spinner><br><span><br>Uploading ' + ($scope.uploaded + 1) + ' of ' + $scope.count + ' files</br><progress id="sell_progress" max="100" value="0" style="width:100%"></progress><br> Saving...</br>'
    });
  };
  ...
  $('#sell_items_select').change(function(event){
    
    for(var i = 0; i < 4; i++){
      $("#sell_image" + (i+1)).fadeOut("fast").attr('src', '');
    }
    $scope.count = event.target.files.length;
    console.log('Total number of files to be uploaded: ', $scope.count);
    for(var i = 0; i < event.target.files.length; i++){
      $("#sell_image" + (i+1)).fadeIn("fast").attr('src',URL.createObjectURL(event.target.files[i]));
    }
  ...
  $scope.Upload = function(){
    ...
    var file = document.getElementById('sell_items_select').files[$scope.uploaded];
  ...
  function progressHandler(event){
    ...
    document.getElementById('sell_progress').value = percent;
  ...
  function completeHandler(event){
    ...
    if($scope.uploaded == $scope.count){
      ...
      var postInfo = {
        ...
        price: $scope.s.price,
        // trade: $scope.s.trade,
        // swap_items: $scope.sell_data.swap_items,
        main_category: $scope.s.main_category,
        sub_category: $scope.s.sub_category
        //tags: $scope.tags
